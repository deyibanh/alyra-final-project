...
const OTHER_ROLE = ethers.utils.keccak256(ethers.utils.toUtf8Bytes("PILOT_ROLE"));
...
describe("SWAccessControl", function () {
    ...
    describe("Granting", function () {
        ...
        it("accounts can be granted a role multiple times", async function () {
            await accessControl.grantRole(ROLE, authorized.address);
            await expect(accessControl.grantRole(ROLE, authorized.address)).to.not.emit(accessControl, "RoleGranted");
        });
    });
    ...
    describe("Revoking", function () {
        it("roles that are not had can be revoked", async function () {
            ...
            await expect(accessControl.revokeRole(ROLE, authorized.address)).to.not.emit(accessControl, "RoleRevoked");
        });
        ...
        context("with granted role", function () {
            ...
            it("a role can be revoked multiple times", async function () {
                ...
                await expect(accessControl.revokeRole(ROLE, authorized.address)).to.not.emit(
                    accessControl,
                    "RoleRevoked"
                );
            });
    ...
    describe("Renouncing", function () {
        it("roles that are not had can be renounced", async function () {
            await expect(accessControl.connect(authorized).renounceRole(ROLE, authorized.address)).to.not.emit(
                accessControl,
                "RoleRevoked"
            );
        });
